...
class DataBase (private val c : Context) : SQLiteOpenHelper(c, databaseName, null, dataBaseVersion) {
    ...
    //    val sqLiteDatabase = this.writableDatabase
    //    val values = ContentValues()
    //    values.put(puth_image, "jui")
    //    val result = sqLiteDatabase.update(table_name_part, values, "$id_part=?", arrayOf(id_part))
    //  Log.d("tag", "result updateImageUrl " + id_part+ "  +  " + result)
    //  val signatureDeleted  = c.resources.getString(R.string.signature_deleted)
    //  val firmaNoEncontada  = c.resources.getString(R.string.firma_no_encontada)
    //  if ( result == 1 ) Toast.makeText(c, signatureDeleted, Toast.LENGTH_LONG).show()
    //  else Toast.makeText(c, firmaNoEncontada, Toast.LENGTH_LONG).show()
    }
